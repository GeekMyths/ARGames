...
namespace BehaviorDesigner.Editor
{
    ...
    public class DLLSelector
    {
        ...
        public static void RemoveAssembly()
        {
            ...
            if (EditorUtility.DisplayDialog("Remove Unnecessary Assembly",
                "Behavior Designer includes an assembly for versions before and after Unity 4.3. Do you want to delete the unnecessary assembly?",
                "Yes", "No")) {

                // Move the gizmos directory into /Assets
                if (Directory.Exists(Application.dataPath + "/Behavior Designer/Gizmos")) {
                    if (Directory.Exists(Application.dataPath + "/Gizmos")) {
                        var files = Directory.GetFiles(Application.dataPath + "/Behavior Designer/Gizmos");
                        for (int i = 0; i < files.Length; ++i) {
                            var fileInfo = new FileInfo(files[i]);
                            if (File.Exists(Application.dataPath + "/Gizmos/" + fileInfo.Name)) {
                                File.Delete(Application.dataPath + "/Gizmos/" + fileInfo.Name);
                            }
                            fileInfo.MoveTo(Application.dataPath + "/Gizmos/" + fileInfo.Name);
                        }
                        Directory.Delete(Application.dataPath + "/Behavior Designer/Gizmos");
                    } else {
                        Directory.Move(Application.dataPath + "/Behavior Designer/Gizmos", Application.dataPath + "/Gizmos");
                    }
                }
...
#else
                EditorUtility.DisplayDialog("Assemblies Removed", "The assemblies were removed. Behavior Designer can be accessed from the Window toolbar.", "OK");
#endif
            } else {
                EditorUtility.DisplayDialog("Assemblies Not Removed",
                    "The assemblies were not removed. You can manually delete the assemblies located within the Behavior Designer and Behavior Designer editor folder. Behavior Designer can be accessed from the Window toolbar.", "OK");
            }
        }
    }
}